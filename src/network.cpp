...
void NetworkClient::connectToWifi(){
    ...
    //logInfo("s|WiFi Firmware version is",WiFi.firmwareVersion());
    //Serial.print((char*)F("WiFi Firmware version is "));
    #ifndef ESP32-POE
      //Serial.println(WiFi.firmwareVersion());
      int status = WL_IDLE_STATUS;
      while ( status != WL_CONNECTED) {
          logInfo("s|Attempting to connect to Wi-Fi SSID: ",wifi_ssid);
          //Serial_printf((char*)F("Attempting to connect to Wi-Fi SSID: %s \n"), wifi_ssid);
          // Connect to WPA/WPA2 network. Change this line if using open or WEP network:
          status = WiFi.begin(wifi_ssid, wifi_password);
          if (status == WL_CONNECTED) {
            logInfo("s|Connected to Wi-Fi SSID: ",wifi_ssid);
